pipeline {
        ...
        stages {
            ...
            stage('pkr validate') {
                when { expression { return params.Terraform == 'Apply'} }       
                steps {
                    sh 'sudo docker run -t --mount type=bind,source=$(pwd)/packer/image.json,target=/mnt/packer/image.json --mount type=bind,source=$(pwd)/ansible/,target=/mnt/ansible/ hashicorp/packer:latest validate /mnt/packer/image.json'
                }
            }                
            stage('pkr build') {
                when { expression { return params.Terraform == 'Apply'} }       
